...
#include "../../asb_uci/base/headers/AccessorFileLocationMT.h"
...
void Example::testOMS83AddAndChoiceMethods(){
    ...
    vst2.addConfiguration({cct1,cct2});
...
void Example::runExample(int argc, char* argv[]) {
 ...
 asb_uci::type::ServiceStatusMT Example::createServiceStatusMT(asb_uci::base::AbstractServiceBusConnection asbs){
    ...
    systemID.setDescriptiveLabel(asbs.getMySystemLabel());
    ...
    ser.setServiceVersion(Example::getVersion());
    
    // Establecer encabezado usando métodos de encadenamiento
    asb_uci::type::HeaderType header;
    header.setSystemID(systemID).setTimestamp(Example::getCurrentTimeMillis()).setSchemaVersion("XML").setMode(uci::type::MessageModeEnum::UCI_SIMULATION).setServiceID(ser);

    asb_uci::type::ServiceStatusMDT messageData;
    ...
    serviceStatus.getSecurityInformation().getOwnerProducer().push_back(usaChoiceType);
...
uci::type::DateTimeTypeValue Example::getCurrentTimeMillis(){
return std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::system_clock::now().time_since_epoch()).count();
}

long long Example::getNanosDuration(){
...
uci::type::DateTimeTypeValue Example::addTime(xs::DateTime baseTime, int64_t seconds, int64_t nanoseconds){
