  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if ((valueType.first == "<xmlattr>") && topLevel) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setFOV_Timestamp(extxml_uci::util::SerializationHelpers::deserializeDateTime(*value));
      }
    } else if (valueType.first == nsPrefix + FOV_ActivityType_Names::MaximumEffectiveRange_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setMaximumEffectiveRange(extxml_uci::util::SerializationHelpers::deserializeDouble(*value));
      }
    }
  }
...
std::string FOV_ActivityType::serialize(const uci::type::FOV_ActivityType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, FOV_ActivityType_Names::Extern_Type_Name);
  }
  ...
  if (accessor.hasFOV_Timestamp()) {
    extxml_uci::util::SerializationHelpers::serializeDateTime(accessor.getFOV_Timestamp(), node, FOV_ActivityType_Names::FOV_Timestamp_Name);
  }
  if (accessor.hasMaximumEffectiveRange()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getMaximumEffectiveRange(), node, FOV_ActivityType_Names::MaximumEffectiveRange_Name);
  }
  if (createNode) {
