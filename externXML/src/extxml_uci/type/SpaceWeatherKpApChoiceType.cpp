  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if ((valueType.first == "<xmlattr>") && topLevel) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setKp(extxml_uci::util::SerializationHelpers::deserializeDouble(*value));
      }
    } else if (valueType.first == nsPrefix + SpaceWeatherKpApChoiceType_Names::Ap_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setAp(extxml_uci::util::SerializationHelpers::deserializeDouble(*value));
      }
    }
  }
  return true;
...
std::string SpaceWeatherKpApChoiceType::serialize(const uci::type::SpaceWeatherKpApChoiceType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, SpaceWeatherKpApChoiceType_Names::Extern_Type_Name);
  }
  if (accessor.isKp()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getKp(), node, SpaceWeatherKpApChoiceType_Names::Kp_Name);
  } else if (accessor.isAp()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getAp(), node, SpaceWeatherKpApChoiceType_Names::Ap_Name);
  }
  if (createNode) {
