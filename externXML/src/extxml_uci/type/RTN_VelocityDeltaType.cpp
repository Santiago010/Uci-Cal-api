...
std::string RTN_VelocityDeltaType::serialize(const uci::type::RTN_VelocityDeltaType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, RTN_VelocityDeltaType_Names::Extern_Type_Name);
  }
  if (accessor.hasRadialDelta()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getRadialDelta(), node, RTN_VelocityDeltaType_Names::RadialDelta_Name);
  }
  if (accessor.hasTransverseDelta()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getTransverseDelta(), node, RTN_VelocityDeltaType_Names::TransverseDelta_Name);
  }
  if (accessor.hasNormalDelta()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getNormalDelta(), node, RTN_VelocityDeltaType_Names::NormalDelta_Name);
  }
  if (createNode) {
