  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if ((valueType.first == "<xmlattr>") && topLevel) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setPowerAtTarget(extxml_uci::util::SerializationHelpers::deserializeDouble(*value));
      }
    } else if (valueType.first == nsPrefix + EA_PowerType_Names::ERP_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setERP(extxml_uci::util::SerializationHelpers::deserializeDouble(*value));
      }
    } else if (valueType.first == nsPrefix + EA_PowerType_Names::JtoS_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setJtoS(extxml_uci::util::SerializationHelpers::deserializeDouble(*value));
      }
    }
  }
  return true;
...
std::string EA_PowerType::serialize(const uci::type::EA_PowerType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, EA_PowerType_Names::Extern_Type_Name);
  }
  if (accessor.isPowerAtTarget()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getPowerAtTarget(), node, EA_PowerType_Names::PowerAtTarget_Name);
  } else if (accessor.isERP()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getERP(), node, EA_PowerType_Names::ERP_Name);
  } else if (accessor.isJtoS()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getJtoS(), node, EA_PowerType_Names::JtoS_Name);
  }
  if (createNode) {
