  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if ((valueType.first == "<xmlattr>") && topLevel) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setFuzeDistance(extxml_uci::util::SerializationHelpers::deserializeDouble(*value));
      }
    } else if (valueType.first == nsPrefix + FuzeTriggerType_Names::FuzeDelayTime_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setFuzeDelayTime(extxml_uci::util::SerializationHelpers::deserializeDuration(*value));
      }
    }
  }
  return true;
...
std::string FuzeTriggerType::serialize(const uci::type::FuzeTriggerType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, FuzeTriggerType_Names::Extern_Type_Name);
  }
  if (accessor.isFuzeDistance()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getFuzeDistance(), node, FuzeTriggerType_Names::FuzeDistance_Name);
  } else if (accessor.isFuzeDelayTime()) {
    extxml_uci::util::SerializationHelpers::serializeDuration(accessor.getFuzeDelayTime(), node, FuzeTriggerType_Names::FuzeDelayTime_Name);
  }
  if (createNode) {
