  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if ((valueType.first == "<xmlattr>") && topLevel) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setMaximumRadiatedERP(extxml_uci::util::SerializationHelpers::deserializeDouble(*value));
      }
    } else if (valueType.first == nsPrefix + EmconERP_Type_Names::RadiateFullPower_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setRadiateFullPower(extxml_uci::util::SerializationHelpers::deserializeBoolean(*value));
      }
    }
  }
  return true;
...
std::string EmconERP_Type::serialize(const uci::type::EmconERP_Type& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, EmconERP_Type_Names::Extern_Type_Name);
  }
  if (accessor.isMaximumRadiatedERP()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getMaximumRadiatedERP(), node, EmconERP_Type_Names::MaximumRadiatedERP_Name);
  } else if (accessor.isRadiateFullPower()) {
    extxml_uci::util::SerializationHelpers::serializeBoolean(accessor.getRadiateFullPower(), node, EmconERP_Type_Names::RadiateFullPower_Name);
  }
  if (createNode) {
