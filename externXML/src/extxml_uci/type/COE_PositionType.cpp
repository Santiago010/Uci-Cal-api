...
bool COE_PositionType::deserialize(const boost::property_tree::ptree& propTree, uci::type::COE_PositionType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if ((valueType.first == "<xmlattr>") && topLevel) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setArgumentOfLatitude(extxml_uci::util::SerializationHelpers::deserializeDouble(*value));
      }
    } else if (valueType.first == nsPrefix + COE_PositionType_Names::TrueLongitude_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setTrueLongitude(extxml_uci::util::SerializationHelpers::deserializeDouble(*value));
      }
    }
  }
  return true;
...
std::string COE_PositionType::serialize(const uci::type::COE_PositionType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, COE_PositionType_Names::Extern_Type_Name);
  }
  if (accessor.isMeanAnomaly()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getMeanAnomaly(), node, COE_PositionType_Names::MeanAnomaly_Name);
  } else if (accessor.isArgumentOfLatitude()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getArgumentOfLatitude(), node, COE_PositionType_Names::ArgumentOfLatitude_Name);
  } else if (accessor.isTrueLongitude()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getTrueLongitude(), node, COE_PositionType_Names::TrueLongitude_Name);
  }
  if (createNode) {
