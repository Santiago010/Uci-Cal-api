  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if ((valueType.first == "<xmlattr>") && topLevel) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setF_StopSetting(extxml_uci::util::SerializationHelpers::deserializeFloat(*value));
      }
    } else if (valueType.first == nsPrefix + PO_ComponentF_StopSettingsType_Names::AutoF_Stop_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setAutoF_Stop(extxml_uci::util::SerializationHelpers::deserializeBoolean(*value));
      }
    }
  }
  return true;
...
std::string PO_ComponentF_StopSettingsType::serialize(const uci::type::PO_ComponentF_StopSettingsType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, PO_ComponentF_StopSettingsType_Names::Extern_Type_Name);
  }
  if (accessor.isF_StopSetting()) {
    extxml_uci::util::SerializationHelpers::serializeFloat(accessor.getF_StopSetting(), node, PO_ComponentF_StopSettingsType_Names::F_StopSetting_Name);
  } else if (accessor.isAutoF_Stop()) {
    extxml_uci::util::SerializationHelpers::serializeBoolean(accessor.getAutoF_Stop(), node, PO_ComponentF_StopSettingsType_Names::AutoF_Stop_Name);
  }
  if (createNode) {
