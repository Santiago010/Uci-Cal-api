  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if ((valueType.first == "<xmlattr>") && topLevel) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setResponseEnabled(extxml_uci::util::SerializationHelpers::deserializeBoolean(*value));
      }
    } else if (valueType.first == nsPrefix + Level2Type_Names::SquitterEnabled_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setSquitterEnabled(extxml_uci::util::SerializationHelpers::deserializeBoolean(*value));
      }
    }
  }
  return true;
...
std::string Level2Type::serialize(const uci::type::Level2Type& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, Level2Type_Names::Extern_Type_Name);
  }
  extxml_uci::util::SerializationHelpers::serializeBoolean(accessor.getResponseEnabled(), node, Level2Type_Names::ResponseEnabled_Name);
  extxml_uci::util::SerializationHelpers::serializeBoolean(accessor.getSquitterEnabled(), node, Level2Type_Names::SquitterEnabled_Name);
  if (createNode) {
