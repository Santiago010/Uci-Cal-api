...
bool PrioritizationListItemType::deserialize(const boost::property_tree::ptree& propTree, uci::type::PrioritizationListItemType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if ((valueType.first == "<xmlattr>") && topLevel) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        ...
        PrioritizationType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
      }
...
std::string PrioritizationListItemType::serialize(const uci::type::PrioritizationListItemType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, PrioritizationListItemType_Names::Extern_Type_Name);
  }
  IdentityKindInstanceType::serialize(accessor.getSubject(), node, PrioritizationListItemType_Names::Subject_Name);
  {
    ...
    for (uci::type::PrioritizationListItemType::Priority::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      PrioritizationType::serialize(boundedList.at(i), node, PrioritizationListItemType_Names::Priority_Name);
    }
  }
