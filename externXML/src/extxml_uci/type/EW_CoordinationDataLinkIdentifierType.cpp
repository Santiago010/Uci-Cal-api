  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if ((valueType.first == "<xmlattr>") && topLevel) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setRequestNumber(extxml_uci::util::SerializationHelpers::deserializeUnsignedInt(*value));
      }
    } else if (valueType.first == nsPrefix + EW_CoordinationDataLinkIdentifierType_Names::Response_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setResponse(extxml_uci::util::SerializationHelpers::deserializeBoolean(*value));
      }
...
std::string EW_CoordinationDataLinkIdentifierType::serialize(const uci::type::EW_CoordinationDataLinkIdentifierType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, EW_CoordinationDataLinkIdentifierType_Names::Extern_Type_Name);
  }
  DataLinkIdentifierPET::serialize(accessor, node, "", false, false, false);
  if (accessor.hasRequestNumber()) {
    extxml_uci::util::SerializationHelpers::serializeUnsignedInt(accessor.getRequestNumber(), node, EW_CoordinationDataLinkIdentifierType_Names::RequestNumber_Name);
  }
  extxml_uci::util::SerializationHelpers::serializeBoolean(accessor.getResponse(), node, EW_CoordinationDataLinkIdentifierType_Names::Response_Name);
