...
bool SMTI_CollectionConstraintsQualityType::deserialize(const boost::property_tree::ptree& propTree, uci::type::SMTI_CollectionConstraintsQualityType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if ((valueType.first == "<xmlattr>") && topLevel) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setCircularErrorProbable90(extxml_uci::util::SerializationHelpers::deserializeDouble(*value));
      }
    }
  }
  return true;
...
std::string SMTI_CollectionConstraintsQualityType::serialize(const uci::type::SMTI_CollectionConstraintsQualityType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, SMTI_CollectionConstraintsQualityType_Names::Extern_Type_Name);
  }
  if (accessor.isMTIIRS()) {
    extxml_uci::util::SerializationHelpers::serializeString(accessor.getMTIIRS(), node, SMTI_CollectionConstraintsQualityType_Names::MTIIRS_Name);
  } else if (accessor.isCircularErrorProbable90()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getCircularErrorProbable90(), node, SMTI_CollectionConstraintsQualityType_Names::CircularErrorProbable90_Name);
  }
  if (createNode) {
