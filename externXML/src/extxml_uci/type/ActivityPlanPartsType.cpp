...
std::string ActivityPlanPartsType::serialize(const uci::type::ActivityPlanPartsType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool checkIfDerivation, const bool /*topLevel*/) {
  ...
  if (!checkIfDerivation || (accessor.getAccessorType() == uci::type::accessorType::activityPlanPartsType)) {
    ...
    if (addTypeAttribute) {
      extxml_uci::util::SerializationHelpers::addTypeAttribute(node, ActivityPlanPartsType_Names::Extern_Type_Name);
    }
    ...
    if (accessor.hasVehicleSettings()) {
      extxml_uci::util::SerializationHelpers::serializeString(accessor.getVehicleSettings(), node, ActivityPlanPartsType_Names::VehicleSettings_Name);
    }
    if (accessor.hasCommsUsage()) {
      extxml_uci::util::SerializationHelpers::serializeString(accessor.getCommsUsage(), node, ActivityPlanPartsType_Names::CommsUsage_Name);
    }
    if (accessor.hasProductTasks()) {
      extxml_uci::util::SerializationHelpers::serializeString(accessor.getProductTasks(), node, ActivityPlanPartsType_Names::ProductTasks_Name);
    }
    if (createNode) {
