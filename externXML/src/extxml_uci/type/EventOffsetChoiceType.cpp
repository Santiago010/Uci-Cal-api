  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if ((valueType.first == "<xmlattr>") && topLevel) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setOffsetTime(extxml_uci::util::SerializationHelpers::deserializeDuration(*value));
      }
    } else if (valueType.first == nsPrefix + EventOffsetChoiceType_Names::OffsetAngle_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setOffsetAngle(extxml_uci::util::SerializationHelpers::deserializeDouble(*value));
      }
...
std::string EventOffsetChoiceType::serialize(const uci::type::EventOffsetChoiceType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, EventOffsetChoiceType_Names::Extern_Type_Name);
  }
  if (accessor.isOffsetTime()) {
    extxml_uci::util::SerializationHelpers::serializeDuration(accessor.getOffsetTime(), node, EventOffsetChoiceType_Names::OffsetTime_Name);
  } else if (accessor.isOffsetAngle()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getOffsetAngle(), node, EventOffsetChoiceType_Names::OffsetAngle_Name);
