...
std::string AIS_Type::serialize(const uci::type::AIS_Type& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, AIS_Type_Names::Extern_Type_Name);
  }
  if (accessor.hasMMSI_Number()) {
    extxml_uci::util::SerializationHelpers::serializeString(accessor.getMMSI_Number(), node, AIS_Type_Names::MMSI_Number_Name);
  }
  if (accessor.hasIMO_Number()) {
    extxml_uci::util::SerializationHelpers::serializeString(accessor.getIMO_Number(), node, AIS_Type_Names::IMO_Number_Name);
  }
  if (accessor.hasVesselName()) {
    extxml_uci::util::SerializationHelpers::serializeString(accessor.getVesselName(), node, AIS_Type_Names::VesselName_Name);
  }
  if (accessor.hasCallSign()) {
    extxml_uci::util::SerializationHelpers::serializeString(accessor.getCallSign(), node, AIS_Type_Names::CallSign_Name);
  }
  if (createNode) {
