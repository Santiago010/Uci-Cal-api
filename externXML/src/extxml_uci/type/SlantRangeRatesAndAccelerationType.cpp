...
std::string SlantRangeRatesAndAccelerationType::serialize(const uci::type::SlantRangeRatesAndAccelerationType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, SlantRangeRatesAndAccelerationType_Names::Extern_Type_Name);
  }
  if (accessor.hasSlantRangeRate()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getSlantRangeRate(), node, SlantRangeRatesAndAccelerationType_Names::SlantRangeRate_Name);
  }
  if (accessor.hasDeltaSlantRangeRate()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getDeltaSlantRangeRate(), node, SlantRangeRatesAndAccelerationType_Names::DeltaSlantRangeRate_Name);
  }
  if (accessor.hasSlantRangeAcceleration()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getSlantRangeAcceleration(), node, SlantRangeRatesAndAccelerationType_Names::SlantRangeAcceleration_Name);
  }
  if (createNode) {
