...
std::string GatewayControlType::serialize(const uci::type::GatewayControlType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, GatewayControlType_Names::Extern_Type_Name);
  }
  if (accessor.hasInhibitTransmit()) {
    extxml_uci::util::SerializationHelpers::serializeBoolean(accessor.getInhibitTransmit(), node, GatewayControlType_Names::InhibitTransmit_Name);
  }
  if (accessor.hasInhibitReceive()) {
    extxml_uci::util::SerializationHelpers::serializeBoolean(accessor.getInhibitReceive(), node, GatewayControlType_Names::InhibitReceive_Name);
  }
  if (accessor.hasInhibitRelay()) {
    extxml_uci::util::SerializationHelpers::serializeBoolean(accessor.getInhibitRelay(), node, GatewayControlType_Names::InhibitRelay_Name);
  }
  if (accessor.hasInstantaneousReceiveBandwidth()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getInstantaneousReceiveBandwidth(), node, GatewayControlType_Names::InstantaneousReceiveBandwidth_Name);
  }
  if (accessor.hasInstantaneousTransmitBandwidth()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getInstantaneousTransmitBandwidth(), node, GatewayControlType_Names::InstantaneousTransmitBandwidth_Name);
  }
  if (createNode) {
