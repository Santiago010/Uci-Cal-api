...
bool RequirementDependencyBaseType::deserialize(const boost::property_tree::ptree& propTree, uci::type::RequirementDependencyBaseType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if ((valueType.first == "<xmlattr>") && topLevel) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setEarliestTime(extxml_uci::util::SerializationHelpers::deserializeDuration(*value));
      }
    } else if (valueType.first == nsPrefix + RequirementDependencyBaseType_Names::LatestTime_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setLatestTime(extxml_uci::util::SerializationHelpers::deserializeDuration(*value));
      }
    }
  }
  return true;
...
std::string RequirementDependencyBaseType::serialize(const uci::type::RequirementDependencyBaseType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool checkIfDerivation, const bool /*topLevel*/) {
  ...
  if (!checkIfDerivation || (accessor.getAccessorType() == uci::type::accessorType::requirementDependencyBaseType)) {
    ...
    if (addTypeAttribute) {
      extxml_uci::util::SerializationHelpers::addTypeAttribute(node, RequirementDependencyBaseType_Names::Extern_Type_Name);
    }
    ...
    if (accessor.hasEarliestTime()) {
      extxml_uci::util::SerializationHelpers::serializeDuration(accessor.getEarliestTime(), node, RequirementDependencyBaseType_Names::EarliestTime_Name);
    }
    if (accessor.hasLatestTime()) {
      extxml_uci::util::SerializationHelpers::serializeDuration(accessor.getLatestTime(), node, RequirementDependencyBaseType_Names::LatestTime_Name);
    }
    if (createNode) {
