...
bool PO_ComponentStatusProductGeneratorType::deserialize(const boost::property_tree::ptree& propTree, uci::type::PO_ComponentStatusProductGeneratorType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if ((valueType.first == "<xmlattr>") && topLevel) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        ...
        boundedList.resize(boundedListSize + 1);
        PO_ComponentStatusOutputProductSettingsType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
      }
    }
  }
  return true;
...
std::string PO_ComponentStatusProductGeneratorType::serialize(const uci::type::PO_ComponentStatusProductGeneratorType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, PO_ComponentStatusProductGeneratorType_Names::Extern_Type_Name);
  }
  PO_ProductGeneratorID_Type::serialize(accessor.getGeneratorID(), node, PO_ComponentStatusProductGeneratorType_Names::GeneratorID_Name);
  {
    ...
    for (uci::type::PO_ComponentStatusProductGeneratorType::FPA_ID::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      PO_FPA_ID_Type::serialize(boundedList.at(i), node, PO_ComponentStatusProductGeneratorType_Names::FPA_ID_Name);
    }
  }
  ...
  }
  {
    ...
    for (uci::type::PO_ComponentStatusProductGeneratorType::OutputProductSettings::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      PO_ComponentStatusOutputProductSettingsType::serialize(boundedList.at(i), node, PO_ComponentStatusProductGeneratorType_Names::OutputProductSettings_Name);
    }
  }
  if (createNode) {
