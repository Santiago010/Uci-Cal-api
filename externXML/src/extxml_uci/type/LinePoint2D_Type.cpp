...
std::string LinePoint2D_Type::serialize(const uci::type::LinePoint2D_Type& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, LinePoint2D_Type_Names::Extern_Type_Name);
  }
  Point2D_Type::serialize(accessor, node, "", false, false, false);
  if (accessor.hasLeftWidth()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getLeftWidth(), node, LinePoint2D_Type_Names::LeftWidth_Name);
  }
  if (accessor.hasRightWidth()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getRightWidth(), node, LinePoint2D_Type_Names::RightWidth_Name);
  }
  if (createNode) {
