...
bool MFP_AddressType::deserialize(const boost::property_tree::ptree& propTree, uci::type::MFP_AddressType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if ((valueType.first == "<xmlattr>") && topLevel) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setSignalProcessingFootprintIndex(extxml_uci::util::SerializationHelpers::deserializeUnsignedInt(*value));
      }
    }
  }
  return true;
...
std::string MFP_AddressType::serialize(const uci::type::MFP_AddressType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, MFP_AddressType_Names::Extern_Type_Name);
  }
  ...
  }
  {
    ...
    for (uci::type::MFP_AddressType::ServiceControlEndpointIndex::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      extxml_uci::util::SerializationHelpers::serializeUnsignedInt(boundedList.at(i), node, MFP_AddressType_Names::ServiceControlEndpointIndex_Name);
    }
  }
  if (accessor.hasSignalProcessingFootprintIndex()) {
    extxml_uci::util::SerializationHelpers::serializeUnsignedInt(accessor.getSignalProcessingFootprintIndex(), node, MFP_AddressType_Names::SignalProcessingFootprintIndex_Name);
  }
  if (createNode) {
