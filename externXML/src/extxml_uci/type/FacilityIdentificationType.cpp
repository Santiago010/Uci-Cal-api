  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if ((valueType.first == "<xmlattr>") && topLevel) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setAFSPC_SiteIdentifier(extxml_uci::util::SerializationHelpers::deserializeUnsignedShort(*value));
      }
    } else if (valueType.first == nsPrefix + FacilityIdentificationType_Names::AFSPC_SensorIdentifier_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setAFSPC_SensorIdentifier(extxml_uci::util::SerializationHelpers::deserializeUnsignedShort(*value));
      }
    } else if (valueType.first == nsPrefix + FacilityIdentificationType_Names::ForeignFacilityKey_Name) {
      extxml_uci::util::DerivedTypesDeserializer::deserialize(valueType.second, accessor.chooseForeignFacilityKey(extxml_uci::util::SerializationHelpers::checkForTypeAttribute(valueType.second, nsPrefix)), nodeName, nsPrefix);
    }
  }
  return true;
}
...
std::string FacilityIdentificationType::serialize(const uci::type::FacilityIdentificationType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, FacilityIdentificationType_Names::Extern_Type_Name);
  }
  if (accessor.isAFSPC_SiteIdentifier()) {
    extxml_uci::util::SerializationHelpers::serializeUnsignedShort(accessor.getAFSPC_SiteIdentifier(), node, FacilityIdentificationType_Names::AFSPC_SiteIdentifier_Name);
  } else if (accessor.isAFSPC_SensorIdentifier()) {
    extxml_uci::util::SerializationHelpers::serializeUnsignedShort(accessor.getAFSPC_SensorIdentifier(), node, FacilityIdentificationType_Names::AFSPC_SensorIdentifier_Name);
