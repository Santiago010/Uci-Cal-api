  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if ((valueType.first == "<xmlattr>") && topLevel) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setVCM_SolarRadiationPressureCoefficient(extxml_uci::util::SerializationHelpers::deserializeDouble(*value));
      }
    } else if (valueType.first == nsPrefix + SolarRadiationPressureCoefficientChoiceType_Names::ReflectivityCoefficient_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setReflectivityCoefficient(extxml_uci::util::SerializationHelpers::deserializeDouble(*value));
      }
    }
  }
  return true;
...
std::string SolarRadiationPressureCoefficientChoiceType::serialize(const uci::type::SolarRadiationPressureCoefficientChoiceType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, SolarRadiationPressureCoefficientChoiceType_Names::Extern_Type_Name);
  }
  if (accessor.isVCM_SolarRadiationPressureCoefficient()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getVCM_SolarRadiationPressureCoefficient(), node, SolarRadiationPressureCoefficientChoiceType_Names::VCM_SolarRadiationPressureCoefficient_Name);
  } else if (accessor.isReflectivityCoefficient()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getReflectivityCoefficient(), node, SolarRadiationPressureCoefficientChoiceType_Names::ReflectivityCoefficient_Name);
  }
  if (createNode) {
