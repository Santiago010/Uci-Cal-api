...
bool SpectralBandType::deserialize(const boost::property_tree::ptree& propTree, uci::type::SpectralBandType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if ((valueType.first == "<xmlattr>") && topLevel) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        extxml_uci::util::SerializationHelpers::deserializeHexBinary(*value, accessor.enableSpectralImage());
      }
    }
  }
  return true;
...
std::string SpectralBandType::serialize(const uci::type::SpectralBandType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, SpectralBandType_Names::Extern_Type_Name);
  }
  ...
  FrequencyRangeType::serialize(accessor.getFrequency(), node, SpectralBandType_Names::Frequency_Name);
  {
    ...
    for (uci::type::SpectralBandType::SpectralDensity::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      extxml_uci::util::SerializationHelpers::serializeDouble(boundedList.at(i), node, SpectralBandType_Names::SpectralDensity_Name);
    }
  }
  if (accessor.hasSpectralImage()) {
    extxml_uci::util::SerializationHelpers::serializeHexBinary(accessor.getSpectralImage(), node, SpectralBandType_Names::SpectralImage_Name);
  }
  if (createNode) {
