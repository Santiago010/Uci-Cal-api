...
bool ComponentDetailedStatusType::deserialize(const boost::property_tree::ptree& propTree, uci::type::ComponentDetailedStatusType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if ((valueType.first == "<xmlattr>") && topLevel) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        ...
        ComponentDetailedStatusInfoType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
      }
    }
  }
  return true;
...
std::string ComponentDetailedStatusType::serialize(const uci::type::ComponentDetailedStatusType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool checkIfDerivation, const bool /*topLevel*/) {
  ...
  if (!checkIfDerivation || (accessor.getAccessorType() == uci::type::accessorType::componentDetailedStatusType)) {
    ...
    if (addTypeAttribute) {
      extxml_uci::util::SerializationHelpers::addTypeAttribute(node, ComponentDetailedStatusType_Names::Extern_Type_Name);
    }
    ...
    if (accessor.hasComponentElementDescription()) {
      extxml_uci::util::SerializationHelpers::serializeString(accessor.getComponentElementDescription(), node, ComponentDetailedStatusType_Names::ComponentElementDescription_Name);
    }
    ...
    }
    {
      ...
      for (uci::type::ComponentDetailedStatusType::ComponentDetailedStatusInfo::size_type i = 0, end = boundedList.size(); i < end; ++i) {
        ComponentDetailedStatusInfoType::serialize(boundedList.at(i), node, ComponentDetailedStatusType_Names::ComponentDetailedStatusInfo_Name);
      }
    }
    if (createNode) {
