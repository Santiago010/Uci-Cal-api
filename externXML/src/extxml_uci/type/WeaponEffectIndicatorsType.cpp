  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if ((valueType.first == "<xmlattr>") && topLevel) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setPayloadDeployed(extxml_uci::util::SerializationHelpers::deserializeBoolean(*value));
      }
    } else if (valueType.first == nsPrefix + WeaponEffectIndicatorsType_Names::Detonated_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setDetonated(extxml_uci::util::SerializationHelpers::deserializeBoolean(*value));
      }
    }
  }
  return true;
...
std::string WeaponEffectIndicatorsType::serialize(const uci::type::WeaponEffectIndicatorsType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, WeaponEffectIndicatorsType_Names::Extern_Type_Name);
  }
  if (accessor.hasPayloadDeployed()) {
    extxml_uci::util::SerializationHelpers::serializeBoolean(accessor.getPayloadDeployed(), node, WeaponEffectIndicatorsType_Names::PayloadDeployed_Name);
  }
  if (accessor.hasDetonated()) {
    extxml_uci::util::SerializationHelpers::serializeBoolean(accessor.getDetonated(), node, WeaponEffectIndicatorsType_Names::Detonated_Name);
  }
  if (createNode) {
