  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if ((valueType.first == "<xmlattr>") && topLevel) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setTimeOfArrival(extxml_uci::util::SerializationHelpers::deserializeDouble(*value));
      }
    } else if (valueType.first == nsPrefix + ArrivalDataVarianceType_Names::TimeDifferenceOfArrival_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setTimeDifferenceOfArrival(extxml_uci::util::SerializationHelpers::deserializeDouble(*value));
      }
...
std::string ArrivalDataVarianceType::serialize(const uci::type::ArrivalDataVarianceType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, ArrivalDataVarianceType_Names::Extern_Type_Name);
  }
  if (accessor.hasTimeOfArrival()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getTimeOfArrival(), node, ArrivalDataVarianceType_Names::TimeOfArrival_Name);
  }
  if (accessor.hasTimeDifferenceOfArrival()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getTimeDifferenceOfArrival(), node, ArrivalDataVarianceType_Names::TimeDifferenceOfArrival_Name);
  }
  if (accessor.hasFrequencyOfArrival()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getFrequencyOfArrival(), node, ArrivalDataVarianceType_Names::FrequencyOfArrival_Name);
  }
  if (accessor.hasFrequencyDifferenceOfArrival()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getFrequencyDifferenceOfArrival(), node, ArrivalDataVarianceType_Names::FrequencyDifferenceOfArrival_Name);
  }
  if (createNode) {
