  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if ((valueType.first == "<xmlattr>") && topLevel) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setGainStepSize(extxml_uci::util::SerializationHelpers::deserializeDouble(*value));
      }
...
std::string ChannelDescriptionType::serialize(const uci::type::ChannelDescriptionType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, ChannelDescriptionType_Names::Extern_Type_Name);
  }
  ...
  if (accessor.hasGainStepSize()) {
    extxml_uci::util::SerializationHelpers::serializeDouble(accessor.getGainStepSize(), node, ChannelDescriptionType_Names::GainStepSize_Name);
  }
  {
    ...
    for (uci::type::ChannelDescriptionType::BandsSupported::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      extxml_uci::util::SerializationHelpers::serializeInt(boundedList.at(i), node, ChannelDescriptionType_Names::BandsSupported_Name);
    }
  }
  if (createNode) {
