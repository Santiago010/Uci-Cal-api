...
std::string PO_ActivityType::serialize(const uci::type::PO_ActivityType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extxml_uci::util::SerializationHelpers::addTypeAttribute(node, PO_ActivityType_Names::Extern_Type_Name);
  }
  ActivityBaseType::serialize(accessor, node, "", false, false, false);
  if (accessor.hasActualStartTime()) {
    extxml_uci::util::SerializationHelpers::serializeDateTime(accessor.getActualStartTime(), node, PO_ActivityType_Names::ActualStartTime_Name);
  }
  if (accessor.hasActualCompletionTime()) {
    extxml_uci::util::SerializationHelpers::serializeDateTime(accessor.getActualCompletionTime(), node, PO_ActivityType_Names::ActualCompletionTime_Name);
  }
  if (accessor.hasAllProductsAndMessagesProduced()) {
    extxml_uci::util::SerializationHelpers::serializeBoolean(accessor.getAllProductsAndMessagesProduced(), node, PO_ActivityType_Names::AllProductsAndMessagesProduced_Name);
  }
  if (accessor.hasFOV()) {
