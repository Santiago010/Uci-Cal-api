...
#include "extxml_uci/util/DerivedTypesTranslator.h"
...
void SerializationHelpers::addNamespaceAttribute(boost::property_tree::ptree& propTree, const std::string& nodeName) {
  boost::property_tree::ptree attr;
  for (const auto& xmlNamespace : xmlNamespaces) {
    attr.add(xmlNamespace.first, xmlNamespace.second);
  }
  boost::property_tree::ptree& node = propTree.get_child(nodeName);
  node.add_child("<xmlattr>", attr);
}

void SerializationHelpers::addTypeAttribute(boost::property_tree::ptree& propTree, const std::string& type) {
  boost::property_tree::ptree attr;
  attr.add("xsi:type", type);
  propTree.add_child("<xmlattr>", attr);
}

std::string SerializationHelpers::checkForNamespacePrefix(const boost::property_tree::ptree& propTree) {
  std::string prefix;
  for (const boost::property_tree::ptree::value_type& valueType : propTree) {
    if (valueType.first.substr(0, 5) == "xmlns") {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        if (*value == uciNamespace) {
          if (valueType.first.length() > 5) {
            prefix = valueType.first.substr(6) + ":";
          }
          break;
        }
      }
...
uci::base::accessorType::AccessorType SerializationHelpers::checkForTypeAttribute(const boost::property_tree::ptree& propTree, const std::string& nsPrefix) {
  const auto typeAttribute = propTree.get_child_optional("<xmlattr>.xsi:type");
  if (typeAttribute) {
...
uci::base::UUID SerializationHelpers::deserializeUUID(const std::string& value) {
  if (value.length() == 32) {
    const std::string valueWithDashes{value.substr(0, 8) + '-' + value.substr(8, 4) + '-' + value.substr(12, 4) + '-' + value.substr(16, 4) + '-' + value.substr(20)};
    return uci::base::UUID::fromString(valueWithDashes);
  }
  throw uci::base::UCIException("Unexpected value found when attempting to deserialize UUID (" + value + ")");
}

void SerializationHelpers::serializeUUID(const uci::base::UUID& accessor, boost::property_tree::ptree& node, const std::string& nodeName) {
  std::string value{accessor.toString()};
  value.erase(std::remove(value.begin(), value.end(), '-'), value.end());
  node.add(nodeName, value);
}
