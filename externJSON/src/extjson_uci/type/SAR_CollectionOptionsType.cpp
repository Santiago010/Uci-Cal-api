...
bool SAR_CollectionOptionsType::deserialize(const boost::property_tree::ptree& propTree, uci::type::SAR_CollectionOptionsType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + SAR_CollectionOptionsType_Names::ATR_Name) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setATR_NumberOfTargets(extjson_uci::util::SerializationHelpers::deserializeUnsignedInt(*value));
      }
    } else if (valueType.first == nsPrefix + SAR_CollectionOptionsType_Names::CommandedResolution_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setCommandedResolution(extjson_uci::util::SerializationHelpers::deserializeDouble(*value));
      }
...
std::string SAR_CollectionOptionsType::serialize(const uci::type::SAR_CollectionOptionsType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, SAR_CollectionOptionsType_Names::Extern_Type_Name);
  }
  RadarCollectionOptionsType::serialize(accessor, node, "", false, false, false);
  if (accessor.hasATR()) {
    extjson_uci::util::SerializationHelpers::serializeBoolean(accessor.getATR(), node, SAR_CollectionOptionsType_Names::ATR_Name);
  }
  if (accessor.hasFTI()) {
    extjson_uci::util::SerializationHelpers::serializeBoolean(accessor.getFTI(), node, SAR_CollectionOptionsType_Names::FTI_Name);
  }
  if (accessor.hasATR_NumberOfTargets()) {
    extjson_uci::util::SerializationHelpers::serializeUnsignedInt(accessor.getATR_NumberOfTargets(), node, SAR_CollectionOptionsType_Names::ATR_NumberOfTargets_Name);
  }
  if (accessor.hasCommandedResolution()) {
    extjson_uci::util::SerializationHelpers::serializeDouble(accessor.getCommandedResolution(), node, SAR_CollectionOptionsType_Names::CommandedResolution_Name);
  }
