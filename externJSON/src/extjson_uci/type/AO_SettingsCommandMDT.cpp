...
bool AO_SettingsCommandMDT::deserialize(const boost::property_tree::ptree& propTree, uci::type::AO_SettingsCommandMDT& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = AO_SettingsCommandMDT_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + AO_SettingsCommandMDT_Names::AO_ComponentSettings_Name) {
      ...
      AO_MessageOutputsEnum::deserializeList(valueType.second, accessor.getMessageOutput());
...
std::string AO_SettingsCommandMDT::serialize(const uci::type::AO_SettingsCommandMDT& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, AO_SettingsCommandMDT_Names::Extern_Type_Name);
  }
  CapabilitySettingsCommandBaseType::serialize(accessor, node, "", false, false, false);
  {
    ...
    for (uci::type::AO_SettingsCommandMDT::AO_ComponentSettings::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      AO_ComponentSettingsType::serialize(boundedList.at(i), node, AO_SettingsCommandMDT_Names::AO_ComponentSettings_Name);
    }
  }
  {
    AO_MessageOutputsEnum::serializeList(accessor.getMessageOutput(), node, AO_SettingsCommandMDT_Names::MessageOutput_Name);
  }
