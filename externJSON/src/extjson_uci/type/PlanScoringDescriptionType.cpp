...
bool PlanScoringDescriptionType::deserialize(const boost::property_tree::ptree& propTree, uci::type::PlanScoringDescriptionType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = PlanScoringDescriptionType_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + PlanScoringDescriptionType_Names::ScoringProcessID_Name) {
...
std::string PlanScoringDescriptionType::serialize(const uci::type::PlanScoringDescriptionType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, PlanScoringDescriptionType_Names::Extern_Type_Name);
  }
  ScoringProcessID_Type::serialize(accessor.getScoringProcessID(), node, PlanScoringDescriptionType_Names::ScoringProcessID_Name);
  {
    OpConstraintEnum::serializeList(accessor.getOpConstraint(), node, PlanScoringDescriptionType_Names::OpConstraint_Name);
  }
  {
    ...
    for (uci::type::PlanScoringDescriptionType::ApplicableTo::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      ApplicabilityType::serialize(boundedList.at(i), node, PlanScoringDescriptionType_Names::ApplicableTo_Name);
    }
  }
  if (createNode) {
