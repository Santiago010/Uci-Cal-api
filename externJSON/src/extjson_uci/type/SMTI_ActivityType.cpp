...
bool SMTI_ActivityType::deserialize(const boost::property_tree::ptree& propTree, uci::type::SMTI_ActivityType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = SMTI_ActivityType_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + SMTI_ActivityType_Names::SubCapability_Name) {
      SMTI_SubCapabilityEnum::deserializeList(valueType.second, accessor.getSubCapability());
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        ...
        boundedList.resize(boundedListSize + 1);
        SMTI_PredictedCollectionsType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
      }
...
std::string SMTI_ActivityType::serialize(const uci::type::SMTI_ActivityType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, SMTI_ActivityType_Names::Extern_Type_Name);
  }
  SharedRF_ApertureActivityBaseType::serialize(accessor, node, "", false, false, false);
  {
    SMTI_SubCapabilityEnum::serializeList(accessor.getSubCapability(), node, SMTI_ActivityType_Names::SubCapability_Name);
  }
  RadarPointingTargetType::serialize(accessor.getRadarPointingTarget(), node, SMTI_ActivityType_Names::RadarPointingTarget_Name);
  {
    ...
    for (uci::type::SMTI_ActivityType::Metrics::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      MTI_ActivityStatusDetailType::serialize(boundedList.at(i), node, SMTI_ActivityType_Names::Metrics_Name);
    }
  }
  ...
  if (accessor.hasPolicyApplied()) {
    extjson_uci::util::SerializationHelpers::serializeBoolean(accessor.getPolicyApplied(), node, SMTI_ActivityType_Names::PolicyApplied_Name);
  }
  extjson_uci::util::SerializationHelpers::serializeBoolean(accessor.getAllProductsAndMessagesProduced(), node, SMTI_ActivityType_Names::AllProductsAndMessagesProduced_Name);
  {
    ...
    for (uci::type::SMTI_ActivityType::PredictedCollections::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      SMTI_PredictedCollectionsType::serialize(boundedList.at(i), node, SMTI_ActivityType_Names::PredictedCollections_Name);
    }
  }
  {
    ...
    for (uci::type::SMTI_ActivityType::ElectronicProtectionOptionsEmployed::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      ForeignKeyType::serialize(boundedList.at(i), node, SMTI_ActivityType_Names::ElectronicProtectionOptionsEmployed_Name);
    }
  }
