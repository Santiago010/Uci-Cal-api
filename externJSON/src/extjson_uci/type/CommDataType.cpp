...
bool CommDataType::deserialize(const boost::property_tree::ptree& propTree, uci::type::CommDataType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + CommDataType_Names::Resource_Name) {
...
std::string CommDataType::serialize(const uci::type::CommDataType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool checkIfDerivation, const bool /*topLevel*/) {
  ...
  if (!checkIfDerivation || (accessor.getAccessorType() == uci::type::accessorType::commDataType)) {
    ...
    if (addTypeAttribute) {
      extjson_uci::util::SerializationHelpers::addTypeAttribute(node, CommDataType_Names::Extern_Type_Name);
    }
    ...
    if (accessor.hasUplinkDataRate()) {
      extjson_uci::util::SerializationHelpers::serializeUnsignedInt(accessor.getUplinkDataRate(), node, CommDataType_Names::UplinkDataRate_Name);
    }
    if (accessor.hasUplinkFrequency()) {
      extjson_uci::util::SerializationHelpers::serializeDouble(accessor.getUplinkFrequency(), node, CommDataType_Names::UplinkFrequency_Name);
    }
    if (accessor.hasDownlinkDataRate()) {
      extjson_uci::util::SerializationHelpers::serializeUnsignedInt(accessor.getDownlinkDataRate(), node, CommDataType_Names::DownlinkDataRate_Name);
    }
    if (accessor.hasDownlinkFrequency()) {
      extjson_uci::util::SerializationHelpers::serializeDouble(accessor.getDownlinkFrequency(), node, CommDataType_Names::DownlinkFrequency_Name);
    }
