...
bool RF_ReportMDT::deserialize(const boost::property_tree::ptree& propTree, uci::type::RF_ReportMDT& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = RF_ReportMDT_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + RF_ReportMDT_Names::RF_ReportID_Name) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        ...
        RF_OperationType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
      }
...
std::string RF_ReportMDT::serialize(const uci::type::RF_ReportMDT& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, RF_ReportMDT_Names::Extern_Type_Name);
  }
  ...
  RF_ReportStateEnum::serialize(accessor.getRF_ReportState(), node, RF_ReportMDT_Names::RF_ReportState_Name, false);
  {
    ...
    for (uci::type::RF_ReportMDT::RF_Operation::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      RF_OperationType::serialize(boundedList.at(i), node, RF_ReportMDT_Names::RF_Operation_Name);
    }
  }
  {
    ...
    for (uci::type::RF_ReportMDT::ActiveProfile::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      RF_ProfileType::serialize(boundedList.at(i), node, RF_ReportMDT_Names::ActiveProfile_Name);
    }
  }
  if (createNode) {
