...
bool CommOutageType::deserialize(const boost::property_tree::ptree& propTree, uci::type::CommOutageType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = CommOutageType_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + CommOutageType_Names::OutageCode_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setOutageCode(extjson_uci::util::SerializationHelpers::deserializeInt(*value));
      }
...
std::string CommOutageType::serialize(const uci::type::CommOutageType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, CommOutageType_Names::Extern_Type_Name);
  }
  CommCoverageDataType::serialize(accessor, node, "", false, false, false);
  if (accessor.hasOutageCode()) {
    extjson_uci::util::SerializationHelpers::serializeInt(accessor.getOutageCode(), node, CommOutageType_Names::OutageCode_Name);
  }
  if (accessor.hasOutageDetails()) {
    extjson_uci::util::SerializationHelpers::serializeString(accessor.getOutageDetails(), node, CommOutageType_Names::OutageDetails_Name);
  }
  if (createNode) {
