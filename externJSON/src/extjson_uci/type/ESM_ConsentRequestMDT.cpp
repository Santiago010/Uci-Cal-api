...
bool ESM_ConsentRequestMDT::deserialize(const boost::property_tree::ptree& propTree, uci::type::ESM_ConsentRequestMDT& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = ESM_ConsentRequestMDT_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + ESM_ConsentRequestMDT_Names::SubsystemID_Name) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        ...
        ESM_ConsentRequestType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
      }
    }
  }
  return true;
...
std::string ESM_ConsentRequestMDT::serialize(const uci::type::ESM_ConsentRequestMDT& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, ESM_ConsentRequestMDT_Names::Extern_Type_Name);
  }
  SubsystemID_Type::serialize(accessor.getSubsystemID(), node, ESM_ConsentRequestMDT_Names::SubsystemID_Name);
  {
    ...
    for (uci::type::ESM_ConsentRequestMDT::ConsentRequest::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      ESM_ConsentRequestType::serialize(boundedList.at(i), node, ESM_ConsentRequestMDT_Names::ConsentRequest_Name);
    }
  }
  if (createNode) {
