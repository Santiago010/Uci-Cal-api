...
bool ResourceRequestOptionType::deserialize(const boost::property_tree::ptree& propTree, uci::type::ResourceRequestOptionType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = ResourceRequestOptionType_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + ResourceRequestOptionType_Names::ResourceRequestOptionIndex_Name) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        ...
        ResourceRequestType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
      }
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setMerit(extjson_uci::util::SerializationHelpers::deserializeUnsignedInt(*value));
      }
...
std::string ResourceRequestOptionType::serialize(const uci::type::ResourceRequestOptionType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, ResourceRequestOptionType_Names::Extern_Type_Name);
  }
  extjson_uci::util::SerializationHelpers::serializeUnsignedInt(accessor.getResourceRequestOptionIndex(), node, ResourceRequestOptionType_Names::ResourceRequestOptionIndex_Name);
  {
    ...
    for (uci::type::ResourceRequestOptionType::ResourceRequest::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      ResourceRequestType::serialize(boundedList.at(i), node, ResourceRequestOptionType_Names::ResourceRequest_Name);
    }
  }
  ...
  }
  {
    ...
    for (uci::type::ResourceRequestOptionType::RequestedFrequencyRange::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      FrequencyRangeType::serialize(boundedList.at(i), node, ResourceRequestOptionType_Names::RequestedFrequencyRange_Name);
    }
  }
  if (accessor.hasMerit()) {
    extjson_uci::util::SerializationHelpers::serializeUnsignedInt(accessor.getMerit(), node, ResourceRequestOptionType_Names::Merit_Name);
  }
