...
bool PO_TrackCapabilityType::deserialize(const boost::property_tree::ptree& propTree, uci::type::PO_TrackCapabilityType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + PO_TrackCapabilityType_Names::TrackOptionID_Name) {
      ...
      PO_TrackCapabilityTypeEnum::deserializeList(valueType.second, accessor.getTrackControlOptions());
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setBlankingVolumeSupported(extjson_uci::util::SerializationHelpers::deserializeBoolean(*value));
      }
    }
  }
  return true;
...
std::string PO_TrackCapabilityType::serialize(const uci::type::PO_TrackCapabilityType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, PO_TrackCapabilityType_Names::Extern_Type_Name);
  }
  TrackOptionsID_Type::serialize(accessor.getTrackOptionID(), node, PO_TrackCapabilityType_Names::TrackOptionID_Name);
  {
    PO_TrackCapabilityTypeEnum::serializeList(accessor.getTrackControlOptions(), node, PO_TrackCapabilityType_Names::TrackControlOptions_Name);
  }
  ...
  if (accessor.hasBlankingVolumeSupported()) {
    extjson_uci::util::SerializationHelpers::serializeBoolean(accessor.getBlankingVolumeSupported(), node, PO_TrackCapabilityType_Names::BlankingVolumeSupported_Name);
  }
  if (createNode) {
