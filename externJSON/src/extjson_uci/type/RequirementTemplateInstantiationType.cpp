...
bool RequirementTemplateInstantiationType::deserialize(const boost::property_tree::ptree& propTree, uci::type::RequirementTemplateInstantiationType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = RequirementTemplateInstantiationType_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + RequirementTemplateInstantiationType_Names::RequirementID_Name) {
...
std::string RequirementTemplateInstantiationType::serialize(const uci::type::RequirementTemplateInstantiationType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, RequirementTemplateInstantiationType_Names::Extern_Type_Name);
  }
  {
    ...
    for (uci::type::RequirementTemplateInstantiationType::RequirementID::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      RequirementInstanceID_ChoiceType::serialize(boundedList.at(i), node, RequirementTemplateInstantiationType_Names::RequirementID_Name);
    }
  }
