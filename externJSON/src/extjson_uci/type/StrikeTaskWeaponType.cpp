...
bool StrikeTaskWeaponType::deserialize(const boost::property_tree::ptree& propTree, uci::type::StrikeTaskWeaponType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = StrikeTaskWeaponType_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + StrikeTaskWeaponType_Names::RequiredWeapon_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setRequiredWeapon(extjson_uci::util::SerializationHelpers::deserializeBoolean(*value));
      }
    }
  }
...
std::string StrikeTaskWeaponType::serialize(const uci::type::StrikeTaskWeaponType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, StrikeTaskWeaponType_Names::Extern_Type_Name);
  }
  WeaponeeringType::serialize(accessor, node, "", false, false, false);
  extjson_uci::util::SerializationHelpers::serializeBoolean(accessor.getRequiredWeapon(), node, StrikeTaskWeaponType_Names::RequiredWeapon_Name);
  if (createNode) {
