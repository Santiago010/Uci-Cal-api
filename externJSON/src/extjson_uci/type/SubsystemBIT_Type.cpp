...
bool SubsystemBIT_Type::deserialize(const boost::property_tree::ptree& propTree, uci::type::SubsystemBIT_Type& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = SubsystemBIT_Type_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + SubsystemBIT_Type_Names::BIT_ID_Name) {
      ...
      extjson_uci::util::SerializationHelpers::deserializeStringList(valueType.second, accessor.getBIT_ItemName());
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setExpectedTestDuration(extjson_uci::util::SerializationHelpers::deserializeDuration(*value));
      }
    }
  }
  return true;
...
std::string SubsystemBIT_Type::serialize(const uci::type::SubsystemBIT_Type& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, SubsystemBIT_Type_Names::Extern_Type_Name);
  }
  ...
  {
    extjson_uci::util::SerializationHelpers::serializeStringList(accessor.getBIT_ItemName(), node, SubsystemBIT_Type_Names::BIT_ItemName_Name);
  }
  {
    ...
    for (uci::type::SubsystemBIT_Type::SubsystemComponentID::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      ComponentID_Type::serialize(boundedList.at(i), node, SubsystemBIT_Type_Names::SubsystemComponentID_Name);
    }
  }
  {
    ...
    for (uci::type::SubsystemBIT_Type::CapabilityID::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      CapabilityID_Type::serialize(boundedList.at(i), node, SubsystemBIT_Type_Names::CapabilityID_Name);
    }
  }
  if (accessor.hasExpectedTestDuration()) {
    extjson_uci::util::SerializationHelpers::serializeDuration(accessor.getExpectedTestDuration(), node, SubsystemBIT_Type_Names::ExpectedTestDuration_Name);
  }
  if (createNode) {
