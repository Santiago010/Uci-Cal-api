...
bool LineType::deserialize(const boost::property_tree::ptree& propTree, uci::type::LineType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + LineType_Names::PointChoice_Name) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setLeftWidth(extjson_uci::util::SerializationHelpers::deserializeDouble(*value));
      }
    } else if (valueType.first == nsPrefix + LineType_Names::RightWidth_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setRightWidth(extjson_uci::util::SerializationHelpers::deserializeDouble(*value));
      }
...
std::string LineType::serialize(const uci::type::LineType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool checkIfDerivation, const bool /*topLevel*/) {
  ...
  if (!checkIfDerivation || (accessor.getAccessorType() == uci::type::accessorType::lineType)) {
    ...
    if (addTypeAttribute) {
      extjson_uci::util::SerializationHelpers::addTypeAttribute(node, LineType_Names::Extern_Type_Name);
    }
    ...
    if (accessor.hasLeftWidth()) {
      extjson_uci::util::SerializationHelpers::serializeDouble(accessor.getLeftWidth(), node, LineType_Names::LeftWidth_Name);
    }
    if (accessor.hasRightWidth()) {
      extjson_uci::util::SerializationHelpers::serializeDouble(accessor.getRightWidth(), node, LineType_Names::RightWidth_Name);
    }
    if (accessor.hasMinAltitude()) {
      extjson_uci::util::SerializationHelpers::serializeDouble(accessor.getMinAltitude(), node, LineType_Names::MinAltitude_Name);
    }
    if (accessor.hasMaxAltitude()) {
      extjson_uci::util::SerializationHelpers::serializeDouble(accessor.getMaxAltitude(), node, LineType_Names::MaxAltitude_Name);
    }
