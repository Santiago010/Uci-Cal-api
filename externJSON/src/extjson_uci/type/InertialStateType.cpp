...
bool InertialStateType::deserialize(const boost::property_tree::ptree& propTree, uci::type::InertialStateType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + InertialStateType_Names::Position_Name) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setLink16PositionQuality(extjson_uci::util::SerializationHelpers::deserializeUnsignedByte(*value));
      }
    }
  }
  return true;
...
std::string InertialStateType::serialize(const uci::type::InertialStateType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, InertialStateType_Names::Extern_Type_Name);
  }
  ...
  if (accessor.hasLink16PositionQuality()) {
    extjson_uci::util::SerializationHelpers::serializeUnsignedByte(accessor.getLink16PositionQuality(), node, InertialStateType_Names::Link16PositionQuality_Name);
  }
  if (createNode) {
