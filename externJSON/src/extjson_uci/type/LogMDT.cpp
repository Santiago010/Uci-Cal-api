...
bool LogMDT::deserialize(const boost::property_tree::ptree& propTree, uci::type::LogMDT& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = LogMDT_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + LogMDT_Names::Timestamp_Name) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setServiceUpTime(extjson_uci::util::SerializationHelpers::deserializeDuration(*value));
      }
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        extjson_uci::util::SerializationHelpers::deserializeHexBinary(*value, accessor.enableData());
      }
    }
  }
  return true;
...
std::string LogMDT::serialize(const uci::type::LogMDT& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, LogMDT_Names::Extern_Type_Name);
  }
  extjson_uci::util::SerializationHelpers::serializeDateTime(accessor.getTimestamp(), node, LogMDT_Names::Timestamp_Name);
  if (accessor.hasServiceUpTime()) {
    extjson_uci::util::SerializationHelpers::serializeDuration(accessor.getServiceUpTime(), node, LogMDT_Names::ServiceUpTime_Name);
  }
  ...
  }
  {
    ...
    for (uci::type::LogMDT::ComponentID::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      ComponentID_Type::serialize(boundedList.at(i), node, LogMDT_Names::ComponentID_Name);
    }
  }
  {
    ...
    for (uci::type::LogMDT::CapabilityID::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      CapabilityID_Type::serialize(boundedList.at(i), node, LogMDT_Names::CapabilityID_Name);
    }
  }
  LogSeverityEnum::serialize(accessor.getSeverity(), node, LogMDT_Names::Severity_Name, false);
  extjson_uci::util::SerializationHelpers::serializeString(accessor.getLabel(), node, LogMDT_Names::Label_Name);
  extjson_uci::util::SerializationHelpers::serializeString(accessor.getDetails(), node, LogMDT_Names::Details_Name);
  if (accessor.hasData()) {
    extjson_uci::util::SerializationHelpers::serializeHexBinary(accessor.getData(), node, LogMDT_Names::Data_Name);
  }
  if (createNode) {
