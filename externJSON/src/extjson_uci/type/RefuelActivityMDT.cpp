...
bool RefuelActivityMDT::deserialize(const boost::property_tree::ptree& propTree, uci::type::RefuelActivityMDT& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = RefuelActivityMDT_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + RefuelActivityMDT_Names::SubsystemID_Name) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        ...
        RefuelActivityType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
      }
    }
  }
  return true;
...
std::string RefuelActivityMDT::serialize(const uci::type::RefuelActivityMDT& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, RefuelActivityMDT_Names::Extern_Type_Name);
  }
  SubsystemID_Type::serialize(accessor.getSubsystemID(), node, RefuelActivityMDT_Names::SubsystemID_Name);
  {
    ...
    for (uci::type::RefuelActivityMDT::Activity::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      RefuelActivityType::serialize(boundedList.at(i), node, RefuelActivityMDT_Names::Activity_Name);
    }
  }
  if (createNode) {
