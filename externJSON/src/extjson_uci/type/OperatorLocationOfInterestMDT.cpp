...
bool OperatorLocationOfInterestMDT::deserialize(const boost::property_tree::ptree& propTree, uci::type::OperatorLocationOfInterestMDT& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = OperatorLocationOfInterestMDT_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + OperatorLocationOfInterestMDT_Names::OperatorLocationOfInterestID_Name) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        ...
        ReceiverIdentificationType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
      }
...
std::string OperatorLocationOfInterestMDT::serialize(const uci::type::OperatorLocationOfInterestMDT& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, OperatorLocationOfInterestMDT_Names::Extern_Type_Name);
  }
  ...
  }
  {
    ...
    for (uci::type::OperatorLocationOfInterestMDT::Destination::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      ReceiverIdentificationType::serialize(boundedList.at(i), node, OperatorLocationOfInterestMDT_Names::Destination_Name);
    }
  }
  if (accessor.hasRemarks()) {
    extjson_uci::util::SerializationHelpers::serializeString(accessor.getRemarks(), node, OperatorLocationOfInterestMDT_Names::Remarks_Name);
  }
  if (createNode) {
