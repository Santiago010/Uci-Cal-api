...
bool DoorStatusType::deserialize(const boost::property_tree::ptree& propTree, uci::type::DoorStatusType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = DoorStatusType_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + DoorStatusType_Names::DoorState_Name) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setPercentOpen(extjson_uci::util::SerializationHelpers::deserializeDouble(*value));
      }
    }
  }
...
std::string DoorStatusType::serialize(const uci::type::DoorStatusType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, DoorStatusType_Names::Extern_Type_Name);
  }
  ...
  extjson_uci::util::SerializationHelpers::serializeBoolean(accessor.getCloseOnHungStore(), node, DoorStatusType_Names::CloseOnHungStore_Name);
  {
    ...
    for (uci::type::DoorStatusType::CapabilityID::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      CapabilityID_Type::serialize(boundedList.at(i), node, DoorStatusType_Names::CapabilityID_Name);
    }
  }
  if (accessor.hasLocked()) {
    extjson_uci::util::SerializationHelpers::serializeBoolean(accessor.getLocked(), node, DoorStatusType_Names::Locked_Name);
  }
  if (accessor.hasPercentOpen()) {
    extjson_uci::util::SerializationHelpers::serializeDouble(accessor.getPercentOpen(), node, DoorStatusType_Names::PercentOpen_Name);
  }
  if (createNode) {
