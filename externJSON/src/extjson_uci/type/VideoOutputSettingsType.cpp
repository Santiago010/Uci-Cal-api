...
bool VideoOutputSettingsType::deserialize(const boost::property_tree::ptree& propTree, uci::type::VideoOutputSettingsType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = VideoOutputSettingsType_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + VideoOutputSettingsType_Names::Enable_Name) {
      ...
      extjson_uci::util::SerializationHelpers::deserializeUnsignedShortList(valueType.second, accessor.getSourceChannel());
      ...
      VideoEncoderOutputType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
...
std::string VideoOutputSettingsType::serialize(const uci::type::VideoOutputSettingsType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, VideoOutputSettingsType_Names::Extern_Type_Name);
  }
  extjson_uci::util::SerializationHelpers::serializeBoolean(accessor.getEnable(), node, VideoOutputSettingsType_Names::Enable_Name);
  {
    extjson_uci::util::SerializationHelpers::serializeUnsignedShortList(accessor.getSourceChannel(), node, VideoOutputSettingsType_Names::SourceChannel_Name);
  }
  {
    ...
    for (uci::type::VideoOutputSettingsType::Output::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      VideoEncoderOutputType::serialize(boundedList.at(i), node, VideoOutputSettingsType_Names::Output_Name);
    }
  }
