...
bool OrbitType::deserialize(const boost::property_tree::ptree& propTree, uci::type::OrbitType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = OrbitType_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + OrbitType_Names::OrbitType_Name) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        ...
        CircleType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
      }
...
std::string OrbitType::serialize(const uci::type::OrbitType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, OrbitType_Names::Extern_Type_Name);
  }
  OrbitEnum::serialize(accessor.getOrbitType(), node, OrbitType_Names::OrbitType_Name, false);
  {
    ...
    for (uci::type::OrbitType::Circle::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      CircleType::serialize(boundedList.at(i), node, OrbitType_Names::Circle_Name);
    }
  }
