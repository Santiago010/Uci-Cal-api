...
bool CommSchedulingRequirementMDT::deserialize(const boost::property_tree::ptree& propTree, uci::type::CommSchedulingRequirementMDT& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = CommSchedulingRequirementMDT_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + CommSchedulingRequirementMDT_Names::RequirementID_Name) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        ...
        RoutePlanReferenceType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
      }
    }
  }
  return true;
...
std::string CommSchedulingRequirementMDT::serialize(const uci::type::CommSchedulingRequirementMDT& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, CommSchedulingRequirementMDT_Names::Extern_Type_Name);
  }
  ...
  CommCoverageUserDataType::serialize(accessor.getCommRequirementData(), node, CommSchedulingRequirementMDT_Names::CommRequirementData_Name);
  {
    ...
    for (uci::type::CommSchedulingRequirementMDT::RequirementRoutePlan::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      RoutePlanReferenceType::serialize(boundedList.at(i), node, CommSchedulingRequirementMDT_Names::RequirementRoutePlan_Name);
    }
  }
  if (createNode) {
