...
bool Point3D_Type::deserialize(const boost::property_tree::ptree& propTree, uci::type::Point3D_Type& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = Point3D_Type_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + Point3D_Type_Names::Latitude_Name) {
...
std::string Point3D_Type::serialize(const uci::type::Point3D_Type& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool checkIfDerivation, const bool /*topLevel*/) {
  ...
  if (!checkIfDerivation || (accessor.getAccessorType() == uci::type::accessorType::point3D_Type)) {
    ...
    if (addTypeAttribute) {
      extjson_uci::util::SerializationHelpers::addTypeAttribute(node, Point3D_Type_Names::Extern_Type_Name);
    }
    extjson_uci::util::SerializationHelpers::serializeDouble(accessor.getLatitude(), node, Point3D_Type_Names::Latitude_Name);
    extjson_uci::util::SerializationHelpers::serializeDouble(accessor.getLongitude(), node, Point3D_Type_Names::Longitude_Name);
    extjson_uci::util::SerializationHelpers::serializeDouble(accessor.getAltitude(), node, Point3D_Type_Names::Altitude_Name);
    ...
    if (accessor.hasTimestamp()) {
      extjson_uci::util::SerializationHelpers::serializeDateTime(accessor.getTimestamp(), node, Point3D_Type_Names::Timestamp_Name);
    }
    if (createNode) {
