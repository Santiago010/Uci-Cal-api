...
bool MissionPlanMDT::deserialize(const boost::property_tree::ptree& propTree, uci::type::MissionPlanMDT& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = MissionPlanMDT_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + MissionPlanMDT_Names::MissionPlanID_Name) {
      ...
      extjson_uci::util::DerivedTypesDeserializer::deserialize(valueType.second, accessor.enableSubPlans(extjson_uci::util::SerializationHelpers::checkForTypeAttribute(valueType.second, nsPrefix)), nodeName, nsPrefix);
...
std::string MissionPlanMDT::serialize(const uci::type::MissionPlanMDT& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, MissionPlanMDT_Names::Extern_Type_Name);
  }
  ...
  }
  {
    ...
    for (uci::type::MissionPlanMDT::ParentMissionPlanID::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      MissionPlanID_Type::serialize(boundedList.at(i), node, MissionPlanMDT_Names::ParentMissionPlanID_Name);
    }
  }
  {
    ...
    for (uci::type::MissionPlanMDT::SubordinateMissionPlanID::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      MissionPlanID_Type::serialize(boundedList.at(i), node, MissionPlanMDT_Names::SubordinateMissionPlanID_Name);
    }
  }
  extjson_uci::util::SerializationHelpers::serializeBoolean(accessor.getForPlanningUseOnly(), node, MissionPlanMDT_Names::ForPlanningUseOnly_Name);
