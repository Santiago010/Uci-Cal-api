...
bool ProductParentType::deserialize(const boost::property_tree::ptree& propTree, uci::type::ProductParentType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = ProductParentType_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + ProductParentType_Names::RequirementID_Name) {
...
std::string ProductParentType::serialize(const uci::type::ProductParentType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, ProductParentType_Names::Extern_Type_Name);
  }
  if (accessor.isRequirementID()) {
  ...
  } else if (accessor.isProductProcessingRequestID()) {
    {
      ...
      for (uci::type::ProductParentType::ProductProcessingRequestID::size_type i = 0, end = boundedList.size(); i < end; ++i) {
        RequestID_Type::serialize(boundedList.at(i), node, ProductParentType_Names::ProductProcessingRequestID_Name);
      }
    }
  }
  if (createNode) {
