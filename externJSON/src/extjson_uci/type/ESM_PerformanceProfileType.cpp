...
bool ESM_PerformanceProfileType::deserialize(const boost::property_tree::ptree& propTree, uci::type::ESM_PerformanceProfileType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = ESM_PerformanceProfileType_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + ESM_PerformanceProfileType_Names::FieldOfRegard_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        ...
        FieldOfRegardType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
      }
...
std::string ESM_PerformanceProfileType::serialize(const uci::type::ESM_PerformanceProfileType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, ESM_PerformanceProfileType_Names::Extern_Type_Name);
  }
  {
    ...
    for (uci::type::ESM_PerformanceProfileType::FieldOfRegard::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      FieldOfRegardType::serialize(boundedList.at(i), node, ESM_PerformanceProfileType_Names::FieldOfRegard_Name);
    }
  }
  {
    ...
    for (uci::type::ESM_PerformanceProfileType::Band::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      FrequencyRangeType::serialize(boundedList.at(i), node, ESM_PerformanceProfileType_Names::Band_Name);
    }
  }
  if (accessor.hasBeamLimit()) {
    extjson_uci::util::SerializationHelpers::serializeUnsignedInt(accessor.getBeamLimit(), node, ESM_PerformanceProfileType_Names::BeamLimit_Name);
  }
  if (accessor.hasFrequencyChangeTime()) {
    extjson_uci::util::SerializationHelpers::serializeDuration(accessor.getFrequencyChangeTime(), node, ESM_PerformanceProfileType_Names::FrequencyChangeTime_Name);
  }
  if (createNode) {
