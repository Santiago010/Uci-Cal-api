...
bool RequirementPlanningResultBaseType::deserialize(const boost::property_tree::ptree& propTree, uci::type::RequirementPlanningResultBaseType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = RequirementPlanningResultBaseType_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + RequirementPlanningResultBaseType_Names::PlanningResult_Name) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        ...
        UnallocatedReasonType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
      }
    }
  }
  return true;
...
std::string RequirementPlanningResultBaseType::serialize(const uci::type::RequirementPlanningResultBaseType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool checkIfDerivation, const bool /*topLevel*/) {
  ...
  if (!checkIfDerivation || (accessor.getAccessorType() == uci::type::accessorType::requirementPlanningResultBaseType)) {
    ...
    if (addTypeAttribute) {
      extjson_uci::util::SerializationHelpers::addTypeAttribute(node, RequirementPlanningResultBaseType_Names::Extern_Type_Name);
    }
    ...
    }
    {
      ...
      for (uci::type::RequirementPlanningResultBaseType::NotAllocatedReason::size_type i = 0, end = boundedList.size(); i < end; ++i) {
        UnallocatedReasonType::serialize(boundedList.at(i), node, RequirementPlanningResultBaseType_Names::NotAllocatedReason_Name);
      }
    }
    if (createNode) {
