...
bool SubsystemCompletedBIT_Type::deserialize(const boost::property_tree::ptree& propTree, uci::type::SubsystemCompletedBIT_Type& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = SubsystemCompletedBIT_Type_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + SubsystemCompletedBIT_Type_Names::BIT_ID_Name) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        ...
        SubsystemCompletedBIT_ItemType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
      }
    }
  }
  return true;
...
std::string SubsystemCompletedBIT_Type::serialize(const uci::type::SubsystemCompletedBIT_Type& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, SubsystemCompletedBIT_Type_Names::Extern_Type_Name);
  }
  BIT_ID_Type::serialize(accessor.getBIT_ID(), node, SubsystemCompletedBIT_Type_Names::BIT_ID_Name);
  extjson_uci::util::SerializationHelpers::serializeDateTime(accessor.getTimetag(), node, SubsystemCompletedBIT_Type_Names::Timetag_Name);
  ...
  if (accessor.hasFailReason()) {
    extjson_uci::util::SerializationHelpers::serializeString(accessor.getFailReason(), node, SubsystemCompletedBIT_Type_Names::FailReason_Name);
  }
  {
    ...
    for (uci::type::SubsystemCompletedBIT_Type::BIT_Item::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      SubsystemCompletedBIT_ItemType::serialize(boundedList.at(i), node, SubsystemCompletedBIT_Type_Names::BIT_Item_Name);
    }
  }
  if (createNode) {
