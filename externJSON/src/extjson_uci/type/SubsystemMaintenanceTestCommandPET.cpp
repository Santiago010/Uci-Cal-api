...
bool SubsystemMaintenanceTestCommandPET::deserialize(const boost::property_tree::ptree& propTree, uci::type::SubsystemMaintenanceTestCommandPET& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = SubsystemMaintenanceTestCommandPET_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + SubsystemMaintenanceTestCommandPET_Names::TestID_Name) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        ...
        SubsystemMaintenanceEventSettingsConfigurationType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
      }
    }
  }
  return true;
...
std::string SubsystemMaintenanceTestCommandPET::serialize(const uci::type::SubsystemMaintenanceTestCommandPET& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool checkIfDerivation, const bool /*topLevel*/) {
  ...
  if (!checkIfDerivation || (accessor.getAccessorType() == uci::type::accessorType::subsystemMaintenanceTestCommandPET)) {
    ...
    if (addTypeAttribute) {
      extjson_uci::util::SerializationHelpers::addTypeAttribute(node, SubsystemMaintenanceTestCommandPET_Names::Extern_Type_Name);
    }
    SubsystemMaintenanceID_Type::serialize(accessor.getTestID(), node, SubsystemMaintenanceTestCommandPET_Names::TestID_Name);
    {
      ...
      for (uci::type::SubsystemMaintenanceTestCommandPET::TestEventSettings::size_type i = 0, end = boundedList.size(); i < end; ++i) {
        SubsystemMaintenanceEventSettingsConfigurationType::serialize(boundedList.at(i), node, SubsystemMaintenanceTestCommandPET_Names::TestEventSettings_Name);
      }
    }
    if (createNode) {
