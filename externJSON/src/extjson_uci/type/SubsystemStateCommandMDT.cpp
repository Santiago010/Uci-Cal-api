...
bool SubsystemStateCommandMDT::deserialize(const boost::property_tree::ptree& propTree, uci::type::SubsystemStateCommandMDT& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = SubsystemStateCommandMDT_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + SubsystemStateCommandMDT_Names::SubsystemID_Name) {
...
std::string SubsystemStateCommandMDT::serialize(const uci::type::SubsystemStateCommandMDT& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, SubsystemStateCommandMDT_Names::Extern_Type_Name);
  }
  ...
  }
  {
    ...
    for (uci::type::SubsystemStateCommandMDT::BIT_ID::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      BIT_ID_Type::serialize(boundedList.at(i), node, SubsystemStateCommandMDT_Names::BIT_ID_Name);
    }
  }
