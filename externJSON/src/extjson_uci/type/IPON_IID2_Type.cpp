...
bool IPON_IID2_Type::deserialize(const boost::property_tree::ptree& propTree, uci::type::IPON_IID2_Type& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = IPON_IID2_Type_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + IPON_IID2_Type_Names::ProgramCode_Name) {
...
std::string IPON_IID2_Type::serialize(const uci::type::IPON_IID2_Type& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, IPON_IID2_Type_Names::Extern_Type_Name);
  }
  extjson_uci::util::SerializationHelpers::serializeString(accessor.getProgramCode(), node, IPON_IID2_Type_Names::ProgramCode_Name);
  extjson_uci::util::SerializationHelpers::serializeString(accessor.getSortieNumber(), node, IPON_IID2_Type_Names::SortieNumber_Name);
  extjson_uci::util::SerializationHelpers::serializeString(accessor.getProjectCode(), node, IPON_IID2_Type_Names::ProjectCode_Name);
  if (createNode) {
