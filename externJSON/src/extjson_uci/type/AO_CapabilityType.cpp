...
bool AO_CapabilityType::deserialize(const boost::property_tree::ptree& propTree, uci::type::AO_CapabilityType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = AO_CapabilityType_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + AO_CapabilityType_Names::CapabilityType_Name) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setEyeSafe(extjson_uci::util::SerializationHelpers::deserializeBoolean(*value));
      }
...
std::string AO_CapabilityType::serialize(const uci::type::AO_CapabilityType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, AO_CapabilityType_Names::Extern_Type_Name);
  }
  ...
  extjson_uci::util::SerializationHelpers::serializeBoolean(accessor.getEyeSafe(), node, AO_CapabilityType_Names::EyeSafe_Name);
  {
    ...
    for (uci::type::AO_CapabilityType::AssociatedComponentID::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      ComponentID_Type::serialize(boundedList.at(i), node, AO_CapabilityType_Names::AssociatedComponentID_Name);
    }
  }
  {
    AO_MessageOutputsEnum::serializeList(accessor.getMessageOutput(), node, AO_CapabilityType_Names::MessageOutput_Name);
  }
  if (createNode) {
