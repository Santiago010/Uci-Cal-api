...
bool ResourceRequestType::deserialize(const boost::property_tree::ptree& propTree, uci::type::ResourceRequestType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = ResourceRequestType_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + ResourceRequestType_Names::DependentActivity_Name) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setMaxPercentInterruption(extjson_uci::util::SerializationHelpers::deserializeDouble(*value));
      }
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        ...
        boundedList.resize(boundedListSize + 1);
        ResourceRequestPeriodType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
      }
...
std::string ResourceRequestType::serialize(const uci::type::ResourceRequestType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, ResourceRequestType_Names::Extern_Type_Name);
  }
  {
    ...
    for (uci::type::ResourceRequestType::DependentActivity::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      DependentActivityType::serialize(boundedList.at(i), node, ResourceRequestType_Names::DependentActivity_Name);
    }
  }
  extjson_uci::util::SerializationHelpers::serializeDouble(accessor.getMaxPercentInterruption(), node, ResourceRequestType_Names::MaxPercentInterruption_Name);
  RF_TypeEnum::serialize(accessor.getRF_Mode(), node, ResourceRequestType_Names::RF_Mode_Name, false);
  {
    ...
    for (uci::type::ResourceRequestType::AccessPeriod::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      ResourceRequestPeriodType::serialize(boundedList.at(i), node, ResourceRequestType_Names::AccessPeriod_Name);
    }
  }
