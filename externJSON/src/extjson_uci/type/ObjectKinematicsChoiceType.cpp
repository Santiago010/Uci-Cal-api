...
bool ObjectKinematicsChoiceType::deserialize(const boost::property_tree::ptree& propTree, uci::type::ObjectKinematicsChoiceType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = ObjectKinematicsChoiceType_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + ObjectKinematicsChoiceType_Names::InertialState_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        ...
        InertialStateType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
      }
    } else if (valueType.first == nsPrefix + ObjectKinematicsChoiceType_Names::OrbitKinematics_Name) {
      extjson_uci::util::DerivedTypesDeserializer::deserialize(valueType.second, accessor.chooseOrbitKinematics(extjson_uci::util::SerializationHelpers::checkForTypeAttribute(valueType.second, nsPrefix)), nodeName, nsPrefix);
    }
  }
  return true;
}
...
std::string ObjectKinematicsChoiceType::serialize(const uci::type::ObjectKinematicsChoiceType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, ObjectKinematicsChoiceType_Names::Extern_Type_Name);
  }
  if (accessor.isInertialState()) {
    {
      ...
      for (uci::type::ObjectKinematicsChoiceType::InertialState::size_type i = 0, end = boundedList.size(); i < end; ++i) {
        InertialStateType::serialize(boundedList.at(i), node, ObjectKinematicsChoiceType_Names::InertialState_Name);
      }
    }
