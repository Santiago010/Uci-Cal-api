...
bool DamagedFunctionType::deserialize(const boost::property_tree::ptree& propTree, uci::type::DamagedFunctionType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + DamagedFunctionType_Names::Function_Name) {
...
std::string DamagedFunctionType::serialize(const uci::type::DamagedFunctionType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool checkIfDerivation, const bool /*topLevel*/) {
  ...
  if (!checkIfDerivation || (accessor.getAccessorType() == uci::type::accessorType::damagedFunctionType)) {
    ...
    if (addTypeAttribute) {
      extjson_uci::util::SerializationHelpers::addTypeAttribute(node, DamagedFunctionType_Names::Extern_Type_Name);
    }
    ...
    if (accessor.hasDuration()) {
      extjson_uci::util::SerializationHelpers::serializeDuration(accessor.getDuration(), node, DamagedFunctionType_Names::Duration_Name);
    }
    if (accessor.hasDetails()) {
      extjson_uci::util::SerializationHelpers::serializeString(accessor.getDetails(), node, DamagedFunctionType_Names::Details_Name);
    }
    if (createNode) {
