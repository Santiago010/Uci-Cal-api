...
bool FaultID_Type::deserialize(const boost::property_tree::ptree& propTree, uci::type::FaultID_Type& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = FaultID_Type_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + FaultID_Type_Names::Name_Name) {
...
std::string FaultID_Type::serialize(const uci::type::FaultID_Type& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, FaultID_Type_Names::Extern_Type_Name);
  }
  ID_Type::serialize(accessor, node, "", false, false, false);
  if (accessor.hasName()) {
    extjson_uci::util::SerializationHelpers::serializeString(accessor.getName(), node, FaultID_Type_Names::Name_Name);
  }
  if (createNode) {
