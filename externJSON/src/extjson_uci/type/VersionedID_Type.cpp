...
bool VersionedID_Type::deserialize(const boost::property_tree::ptree& propTree, uci::type::VersionedID_Type& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + VersionedID_Type_Names::Version_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setVersion(extjson_uci::util::SerializationHelpers::deserializeUnsignedInt(*value));
      }
    }
  }
...
std::string VersionedID_Type::serialize(const uci::type::VersionedID_Type& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool checkIfDerivation, const bool /*topLevel*/) {
  ...
  if (!checkIfDerivation || (accessor.getAccessorType() == uci::type::accessorType::versionedID_Type)) {
    ...
    if (addTypeAttribute) {
      extjson_uci::util::SerializationHelpers::addTypeAttribute(node, VersionedID_Type_Names::Extern_Type_Name);
    }
    ID_Type::serialize(accessor, node, "", false, false, false);
    if (accessor.hasVersion()) {
      extjson_uci::util::SerializationHelpers::serializeUnsignedInt(accessor.getVersion(), node, VersionedID_Type_Names::Version_Name);
    }
    if (createNode) {
