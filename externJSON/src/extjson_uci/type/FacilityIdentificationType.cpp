...
bool FacilityIdentificationType::deserialize(const boost::property_tree::ptree& propTree, uci::type::FacilityIdentificationType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = FacilityIdentificationType_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + FacilityIdentificationType_Names::AFSPC_SiteIdentifier_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setAFSPC_SiteIdentifier(extjson_uci::util::SerializationHelpers::deserializeUnsignedShort(*value));
      }
    } else if (valueType.first == nsPrefix + FacilityIdentificationType_Names::AFSPC_SensorIdentifier_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setAFSPC_SensorIdentifier(extjson_uci::util::SerializationHelpers::deserializeUnsignedShort(*value));
      }
    } else if (valueType.first == nsPrefix + FacilityIdentificationType_Names::ForeignFacilityKey_Name) {
      extjson_uci::util::DerivedTypesDeserializer::deserialize(valueType.second, accessor.chooseForeignFacilityKey(extjson_uci::util::SerializationHelpers::checkForTypeAttribute(valueType.second, nsPrefix)), nodeName, nsPrefix);
    }
  }
  return true;
}
...
std::string FacilityIdentificationType::serialize(const uci::type::FacilityIdentificationType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, FacilityIdentificationType_Names::Extern_Type_Name);
  }
  if (accessor.isAFSPC_SiteIdentifier()) {
    extjson_uci::util::SerializationHelpers::serializeUnsignedShort(accessor.getAFSPC_SiteIdentifier(), node, FacilityIdentificationType_Names::AFSPC_SiteIdentifier_Name);
  } else if (accessor.isAFSPC_SensorIdentifier()) {
    extjson_uci::util::SerializationHelpers::serializeUnsignedShort(accessor.getAFSPC_SensorIdentifier(), node, FacilityIdentificationType_Names::AFSPC_SensorIdentifier_Name);
