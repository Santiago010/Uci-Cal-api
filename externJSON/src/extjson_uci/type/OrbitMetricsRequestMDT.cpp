...
bool OrbitMetricsRequestMDT::deserialize(const boost::property_tree::ptree& propTree, uci::type::OrbitMetricsRequestMDT& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = OrbitMetricsRequestMDT_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + OrbitMetricsRequestMDT_Names::OrbitPlanID_Name) {
...
std::string OrbitMetricsRequestMDT::serialize(const uci::type::OrbitMetricsRequestMDT& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, OrbitMetricsRequestMDT_Names::Extern_Type_Name);
  }
  ...
  {
    OrbitPlanPartsEnum::serializeList(accessor.getOrbitParts(), node, OrbitMetricsRequestMDT_Names::OrbitParts_Name);
  }
  {
    ...
    for (uci::type::OrbitMetricsRequestMDT::OrbitActivityPlanID::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      OrbitActivityPlanID_Type::serialize(boundedList.at(i), node, OrbitMetricsRequestMDT_Names::OrbitActivityPlanID_Name);
    }
  }
  {
    ActivityPlanPartsEnum::serializeList(accessor.getActivityPlanParts(), node, OrbitMetricsRequestMDT_Names::ActivityPlanParts_Name);
  }
  if (createNode) {
