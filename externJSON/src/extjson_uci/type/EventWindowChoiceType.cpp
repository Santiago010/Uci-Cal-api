...
bool EventWindowChoiceType::deserialize(const boost::property_tree::ptree& propTree, uci::type::EventWindowChoiceType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  ...
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + EventWindowChoiceType_Names::WindowAngle_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setWindowAngle(extjson_uci::util::SerializationHelpers::deserializeDouble(*value));
      }
    } else if (valueType.first == nsPrefix + EventWindowChoiceType_Names::WindowDuration_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setWindowDuration(extjson_uci::util::SerializationHelpers::deserializeDuration(*value));
      }
    } else if (valueType.first == nsPrefix + EventWindowChoiceType_Names::WindowRadius_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setWindowRadius(extjson_uci::util::SerializationHelpers::deserializeDouble(*value));
      }
    }
  }
  return true;
...
std::string EventWindowChoiceType::serialize(const uci::type::EventWindowChoiceType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, EventWindowChoiceType_Names::Extern_Type_Name);
  }
  if (accessor.isWindowAngle()) {
    extjson_uci::util::SerializationHelpers::serializeDouble(accessor.getWindowAngle(), node, EventWindowChoiceType_Names::WindowAngle_Name);
  } else if (accessor.isWindowDuration()) {
    extjson_uci::util::SerializationHelpers::serializeDuration(accessor.getWindowDuration(), node, EventWindowChoiceType_Names::WindowDuration_Name);
  } else if (accessor.isWindowRadius()) {
    extjson_uci::util::SerializationHelpers::serializeDouble(accessor.getWindowRadius(), node, EventWindowChoiceType_Names::WindowRadius_Name);
  }
  if (createNode) {
