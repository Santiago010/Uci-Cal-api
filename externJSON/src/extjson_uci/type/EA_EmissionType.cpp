...
bool EA_EmissionType::deserialize(const boost::property_tree::ptree& propTree, uci::type::EA_EmissionType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = EA_EmissionType_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + EA_EmissionType_Names::TechniqueIdentifier_Name) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        ...
        EA_TargetType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
      }
    } else if (valueType.first == nsPrefix + EA_EmissionType_Names::ChannelAttenuation_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setChannelAttenuation(extjson_uci::util::SerializationHelpers::deserializeDouble(*value));
      }
    }
  }
  return true;
...
std::string EA_EmissionType::serialize(const uci::type::EA_EmissionType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, EA_EmissionType_Names::Extern_Type_Name);
  }
  ...
  }
  {
    ...
    for (uci::type::EA_EmissionType::Target::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      EA_TargetType::serialize(boundedList.at(i), node, EA_EmissionType_Names::Target_Name);
    }
  }
  if (accessor.hasChannelAttenuation()) {
    extjson_uci::util::SerializationHelpers::serializeDouble(accessor.getChannelAttenuation(), node, EA_EmissionType_Names::ChannelAttenuation_Name);
  }
  if (createNode) {
