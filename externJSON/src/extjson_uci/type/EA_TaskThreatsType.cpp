...
bool EA_TaskThreatsType::deserialize(const boost::property_tree::ptree& propTree, uci::type::EA_TaskThreatsType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = EA_TaskThreatsType_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + EA_TaskThreatsType_Names::SuppressAll_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setSuppressAll(extjson_uci::util::SerializationHelpers::deserializeBoolean(*value));
      }
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        ...
        EA_TaskSuppressionConstraintsType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
      }
    }
  }
  return true;
...
std::string EA_TaskThreatsType::serialize(const uci::type::EA_TaskThreatsType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, EA_TaskThreatsType_Names::Extern_Type_Name);
  }
  if (accessor.isSuppressAll()) {
    extjson_uci::util::SerializationHelpers::serializeBoolean(accessor.getSuppressAll(), node, EA_TaskThreatsType_Names::SuppressAll_Name);
  } else if (accessor.isSuppressionConstraints()) {
    {
      ...
      for (uci::type::EA_TaskThreatsType::SuppressionConstraints::size_type i = 0, end = boundedList.size(); i < end; ++i) {
        EA_TaskSuppressionConstraintsType::serialize(boundedList.at(i), node, EA_TaskThreatsType_Names::SuppressionConstraints_Name);
      }
    }
  }
  if (createNode) {
