...
bool RF_TaskNormalizedMetricsType::deserialize(const boost::property_tree::ptree& propTree, uci::type::RF_TaskNormalizedMetricsType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = RF_TaskNormalizedMetricsType_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + RF_TaskNormalizedMetricsType_Names::CapabilityID_Name) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        ...
        boundedList.resize(boundedListSize + 1);
        NormalizationTableType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
      }
    }
  }
  return true;
...
std::string RF_TaskNormalizedMetricsType::serialize(const uci::type::RF_TaskNormalizedMetricsType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, RF_TaskNormalizedMetricsType_Names::Extern_Type_Name);
  }
  {
    ...
    for (uci::type::RF_TaskNormalizedMetricsType::CapabilityID::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      CapabilityID_Type::serialize(boundedList.at(i), node, RF_TaskNormalizedMetricsType_Names::CapabilityID_Name);
    }
  }
  ...
  NormalizationInterpolationEnum::serialize(accessor.getInterpolationEnum(), node, RF_TaskNormalizedMetricsType_Names::InterpolationEnum_Name, false);
  {
    ...
    for (uci::type::RF_TaskNormalizedMetricsType::NormalizationElements::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      NormalizationTableType::serialize(boundedList.at(i), node, RF_TaskNormalizedMetricsType_Names::NormalizationElements_Name);
    }
  }
  if (createNode) {
