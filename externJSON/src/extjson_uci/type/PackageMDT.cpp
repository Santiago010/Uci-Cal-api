...
bool PackageMDT::deserialize(const boost::property_tree::ptree& propTree, uci::type::PackageMDT& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = PackageMDT_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + PackageMDT_Names::PackageID_Name) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        ...
        PackagePartnerBaseType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
      }
    }
  }
...
std::string PackageMDT::serialize(const uci::type::PackageMDT& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, PackageMDT_Names::Extern_Type_Name);
  }
  ...
  if (accessor.hasPackageName()) {
    extjson_uci::util::SerializationHelpers::serializeString(accessor.getPackageName(), node, PackageMDT_Names::PackageName_Name);
  }
  ...
  }
  {
    ...
    for (uci::type::PackageMDT::PackagePartner::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      PackagePartnerBaseType::serialize(boundedList.at(i), node, PackageMDT_Names::PackagePartner_Name);
    }
  }
  if (createNode) {
