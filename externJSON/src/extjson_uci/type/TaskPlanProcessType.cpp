...
bool TaskPlanProcessType::deserialize(const boost::property_tree::ptree& propTree, uci::type::TaskPlanProcessType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = TaskPlanProcessType_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + TaskPlanProcessType_Names::DefaultPlanningProcessID_Name) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        ...
        TaskPlanProcessDescriptionType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
      }
    }
  }
  return true;
...
std::string TaskPlanProcessType::serialize(const uci::type::TaskPlanProcessType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, TaskPlanProcessType_Names::Extern_Type_Name);
  }
  PlanningProcessID_Type::serialize(accessor.getDefaultPlanningProcessID(), node, TaskPlanProcessType_Names::DefaultPlanningProcessID_Name);
  {
    ...
    for (uci::type::TaskPlanProcessType::Process::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      TaskPlanProcessDescriptionType::serialize(boundedList.at(i), node, TaskPlanProcessType_Names::Process_Name);
    }
  }
  if (createNode) {
