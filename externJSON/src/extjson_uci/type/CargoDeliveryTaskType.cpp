...
bool CargoDeliveryTaskType::deserialize(const boost::property_tree::ptree& propTree, uci::type::CargoDeliveryTaskType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = CargoDeliveryTaskType_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + CargoDeliveryTaskType_Names::Pickup_Name) {
      ...
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        ...
        CargoTransitionType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
      }
    }
  }
  return true;
...
std::string CargoDeliveryTaskType::serialize(const uci::type::CargoDeliveryTaskType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, CargoDeliveryTaskType_Names::Extern_Type_Name);
  }
  if (accessor.isPickup()) {
  ...
  } else if (accessor.isDropoff()) {
    {
      ...
      for (uci::type::CargoDeliveryTaskType::Dropoff::size_type i = 0, end = boundedList.size(); i < end; ++i) {
        CargoTransitionType::serialize(boundedList.at(i), node, CargoDeliveryTaskType_Names::Dropoff_Name);
      }
    }
  }
  if (createNode) {
