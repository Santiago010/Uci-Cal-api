...
bool AMTI_TaskBaseType::deserialize(const boost::property_tree::ptree& propTree, uci::type::AMTI_TaskBaseType& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = AMTI_TaskBaseType_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + AMTI_TaskBaseType_Names::CapabilityType_Name) {
...
std::string AMTI_TaskBaseType::serialize(const uci::type::AMTI_TaskBaseType& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool checkIfDerivation, const bool /*topLevel*/) {
  ...
  if (!checkIfDerivation || (accessor.getAccessorType() == uci::type::accessorType::aMTI_TaskBaseType)) {
    ...
    if (addTypeAttribute) {
      extjson_uci::util::SerializationHelpers::addTypeAttribute(node, AMTI_TaskBaseType_Names::Extern_Type_Name);
    }
    AMTI_CapabilityEnum::serialize(accessor.getCapabilityType(), node, AMTI_TaskBaseType_Names::CapabilityType_Name, false);
    {
      AMTI_SubCapabilityEnum::serializeList(accessor.getSubCapabilityType(), node, AMTI_TaskBaseType_Names::SubCapabilityType_Name);
    }
    ...
    }
    {
      ...
      for (uci::type::AMTI_TaskBaseType::Output::size_type i = 0, end = boundedList.size(); i < end; ++i) {
        ProductOutputCommandBasicType::serialize(boundedList.at(i), node, AMTI_TaskBaseType_Names::Output_Name);
      }
    }
    if (createNode) {
