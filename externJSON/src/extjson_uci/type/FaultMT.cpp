...
bool FaultMT::deserialize(const boost::property_tree::ptree& propTree, uci::type::FaultMT& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = (topLevel ? FaultMT_Names::Extern_Top_Type_Name : FaultMT_Names::Extern_Type_Name);
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + FaultMT_Names::ObjectState_Name) {
...
std::string FaultMT::serialize(const uci::type::FaultMT& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool topLevel) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, (topLevel ? FaultMT_Names::Extern_Top_Type_Name : FaultMT_Names::Extern_Type_Name));
  }
