...
bool QueryDataRequestMDT::deserialize(const boost::property_tree::ptree& propTree, uci::type::QueryDataRequestMDT& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = QueryDataRequestMDT_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + QueryDataRequestMDT_Names::ResultsInNative_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        ...
        QueryTopicPairType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
      }
    } else if (valueType.first == nsPrefix + QueryDataRequestMDT_Names::QueryResultLimit_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setQueryResultLimit(extjson_uci::util::SerializationHelpers::deserializeUnsignedInt(*value));
      }
    } else if (valueType.first == nsPrefix + QueryDataRequestMDT_Names::ResultsPerPage_Name) {
      if (const boost::optional<std::string> value = valueType.second.get_value_optional<std::string>()) {
        accessor.setResultsPerPage(extjson_uci::util::SerializationHelpers::deserializeUnsignedShort(*value));
      }
...
std::string QueryDataRequestMDT::serialize(const uci::type::QueryDataRequestMDT& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, QueryDataRequestMDT_Names::Extern_Type_Name);
  }
  RequestBaseType::serialize(accessor, node, "", false, false, false);
  {
    ...
    for (uci::type::QueryDataRequestMDT::ResultsInNative::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      QueryTopicPairType::serialize(boundedList.at(i), node, QueryDataRequestMDT_Names::ResultsInNative_Name);
    }
  }
  if (accessor.hasQueryResultLimit()) {
    extjson_uci::util::SerializationHelpers::serializeUnsignedInt(accessor.getQueryResultLimit(), node, QueryDataRequestMDT_Names::QueryResultLimit_Name);
  }
  if (accessor.hasResultsPerPage()) {
    extjson_uci::util::SerializationHelpers::serializeUnsignedShort(accessor.getResultsPerPage(), node, QueryDataRequestMDT_Names::ResultsPerPage_Name);
  }
  if (accessor.hasMostRecentMessagesOnly()) {
    extjson_uci::util::SerializationHelpers::serializeString(accessor.getMostRecentMessagesOnly(), node, QueryDataRequestMDT_Names::MostRecentMessagesOnly_Name);
  }
  if (accessor.hasQueryIdentifiersOnly()) {
    extjson_uci::util::SerializationHelpers::serializeString(accessor.getQueryIdentifiersOnly(), node, QueryDataRequestMDT_Names::QueryIdentifiersOnly_Name);
  }
