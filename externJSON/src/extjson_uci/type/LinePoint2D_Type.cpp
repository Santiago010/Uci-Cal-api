...
bool LinePoint2D_Type::deserialize(const boost::property_tree::ptree& propTree, uci::type::LinePoint2D_Type& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = LinePoint2D_Type_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + LinePoint2D_Type_Names::LeftWidth_Name) {
...
std::string LinePoint2D_Type::serialize(const uci::type::LinePoint2D_Type& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, LinePoint2D_Type_Names::Extern_Type_Name);
  }
  Point2D_Type::serialize(accessor, node, "", false, false, false);
  if (accessor.hasLeftWidth()) {
    extjson_uci::util::SerializationHelpers::serializeDouble(accessor.getLeftWidth(), node, LinePoint2D_Type_Names::LeftWidth_Name);
  }
  if (accessor.hasRightWidth()) {
    extjson_uci::util::SerializationHelpers::serializeDouble(accessor.getRightWidth(), node, LinePoint2D_Type_Names::RightWidth_Name);
  }
  if (createNode) {
