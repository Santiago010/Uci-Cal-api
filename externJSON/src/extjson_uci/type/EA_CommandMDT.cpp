...
bool EA_CommandMDT::deserialize(const boost::property_tree::ptree& propTree, uci::type::EA_CommandMDT& accessor, std::string& nodeName, std::string& nsPrefix, const bool topLevel) {
  nodeName = EA_CommandMDT_Names::Extern_Type_Name;
  const boost::property_tree::ptree& node = (topLevel ? propTree.ordered_begin()->second : propTree);
  for (const boost::property_tree::ptree::value_type& valueType : node.get_child("")) {
    if (valueType.first == nsPrefix + EA_CommandMDT_Names::Command_Name) {
      ...
      EA_CommandType::deserialize(valueType.second, boundedList.at(boundedListSize), nodeName, nsPrefix);
    }
  }
  return true;
}
...
std::string EA_CommandMDT::serialize(const uci::type::EA_CommandMDT& accessor, boost::property_tree::ptree& propTree, const std::string& nodeName, const bool createNode, const bool addTypeAttribute, const bool /*checkIfDerivation*/, const bool /*topLevel*/) {
  ...
  if (addTypeAttribute) {
    extjson_uci::util::SerializationHelpers::addTypeAttribute(node, EA_CommandMDT_Names::Extern_Type_Name);
  }
  {
    ...
    for (uci::type::EA_CommandMDT::Command::size_type i = 0, end = boundedList.size(); i < end; ++i) {
      EA_CommandType::serialize(boundedList.at(i), node, EA_CommandMDT_Names::Command_Name);
    }
  }
  if (createNode) {
